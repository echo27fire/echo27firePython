            ...
            if file.lower().endswith((".png", ".jpg", ".jpeg")):
                ...
                try:
                    ...
                    if not os.path.exists(target_dir):
                        print(
                            f"Target directory {target_dir} does not exist. Skipping."
                        )
                        continue
                    ...
                    while os.path.exists(target_file_path):
                        ...
                        target_file_path = os.path.join(
                            target_dir, f"{filename}_{counter}{file_extension}"
                        )
                        counter += 1
            ...
            if file.lower().endswith((".png", ".jpg", ".jpeg")):
                ...
                try:
                    with Image.open(full_path) as img:
                        ...
                        print(
                            f"Processing: {full_path} (Dimensions: {width}x{height})"
                        )  # Debugging line

                        file_stat = os.stat(full_path)
                        creation_time = file_stat.st_ctime
                        two_levels_up = os.path.basename(
                        ...
                        new_full_path = os.path.join(
                            os.path.dirname(full_path), new_file_name
                        )
        ...
        for file in files:
            if file.lower().endswith((".png", ".jpg", ".jpeg")):
                ...
                try:
                    ...
                    if (
                        uw_ratio * (1 - margin_of_error)
                        <= aspect_ratio
                        <= uw_ratio * (1 + margin_of_error)
                    ) or (
                        super_uw_ratio * (1 - margin_of_error)
                        <= aspect_ratio
                        <= super_uw_ratio * (1 + margin_of_error)
                    ):
                        ...
                        print(
                            f"Moved {file} to {target_dir}"
                        )  # Typo corrected from 'moved' to 'Moved'
                except Exception as e:
                    print(f"Error reading {image_path}: {e}")
